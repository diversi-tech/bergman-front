...
//זה עם הפונקציות למחיקה ולעריכה
...
//         try {
//             if (itemToDelete) {
//                 await OptionsAxios.deleteOptionById(itemToDelete.id); // מחיקת הנתון מהמסד נתונים
//                 setOptions(options.filter(option => option.id !== itemToDelete.id)); // עדכון הרשימה המקומית
//                 dispatch(FillOptionData(options.filter(option => option.id !== itemToDelete.id))); // עדכון ה-state הגלובלי
//                 setDeleteDialogOpen(false); // סגירת דיאלוג המחיקה
//                 setItemToDelete(null); // ניקוי האובייקט שנבחר למחיקה
//             }
//         } catch (error) {
