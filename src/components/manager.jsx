...
import { FETCHAllUsers, FillData } from '../redux/action/userAction';
import userTypeAxios from '../axios/userTypeAxios';
import { FETCHAllUserTypes } from '../redux/action/userTypeAction';
...
export const Manager = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        dispatch(FETCHAllUserTypes(data1));
        setUserType(data1);
        setManagers(managers1);
        dispatch(FETCHAllUsers(managers1));
        setShowTable(false)
      } catch (error) {
        ...
        <Table>
          <TableHead>
            <TableRow style={{ backgroundColor: '#1976d2', color: 'white' }}>
              <TableCell style={{ color: 'white', fontWeight: 'bold', textAlign: 'center' }}>שם משתמש</TableCell>
              <TableCell style={{ color: 'white', fontWeight: 'bold', textAlign: 'center' }}>אימייל</TableCell>
              <TableCell style={{ color: 'white', fontWeight: 'bold', textAlign: 'center' }}>סוג משתמש</TableCell>
              <TableCell style={{ color: 'white', fontWeight: 'bold', textAlign: 'center' }}></TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {managers.map((manager) => (
              <TableRow key={manager.userId}>
                <TableCell >
                  <Avatar style={{textAlign:'center'  }}>{manager.username[0]}</Avatar> {manager.username}
                </TableCell>
  ...
  return (
    <>
      ...
      <Dialog open={openEdit} onClose={handleEditClose} dir="rtl">
      ...
      <Dialog open={openAdd} onClose={handleAddClose} dir="rtl">
