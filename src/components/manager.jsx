...
export const Manager = () => {
  ...
  const handleUserTypeEditChange = (e) => {
    ...
    const selectedUserType = userTypes.find(
      (type) => type.id === selectedUserTypeId
    );
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      setManagers(managers1);
      dispatch(FillUsersData(managers1));
      setSnackbarMessage(`המשתמש $
{currentManager.person.firstName} $
{currentManager.person.lastName} עודכן בהצלחה`);
      setSnackbarOpen(true);
    } catch (error) {
  ...
  const handleAddSubmit = async () => {
    ...
    try {
      ...
      setManagers(managers1);
      dispatch(FillUsersData(managers1));
      setSnackbarMessage(`המשתמש ${newManager.username} נוסף 
בהצלחה`);
      setSnackbarOpen(true);
    } catch (error) {
                ...
                <TableRow key={manager.userId}>
                  ...
                  >
                    <Box display="flex" alignItems="center">
                      <Avatar>
                        {manager.person.firstName[0]}
                        {manager.person.lastName[0]}
                      </Avatar>
                  ...
                  <TableCell padding="none">
                    <Tooltip title="לשליחת אימייל">
                      <IconButton
                        color="primary"
                        onClick={() =>
                          handleEmailDialogOpen(manager.email || "")
                        }
                      >
                        <MailIcon />
                      </IconButton>
                  ...
                  <TableCell padding="0px 20px">
                    <Tooltip title="מחיקת מנהל">
                      <IconButton
                        color="primary"
                        onClick={() => handleDeleteWarningOpen(manager)}
                      >
                        <DeleteIcon />
                      </IconButton>
                    </Tooltip>
  ...
  return (
    <>
      ...
      <Dialog
        ...
        dir="rtl" //style={{ textAlign: 'center' }}
        fullWidth
        maxWidth="sm"
        PaperProps={{
          style: {
        ...
        <DialogContent>
          ...
          <Autocomplete
            ...
            options={emailDomains.map(
              (domain) => `$
{newManager.email.split('@')[0]}@${domain}`
            )}
            renderInput={(params) => (
              <CacheProvider value={cacheRtl}>
                <ThemeProvider theme={theme}>
...
password" // הוסף את התכונה הזו
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            האם אתה בטוח שאתה רוצה למחוק את המנהל הזה?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button
...
style={{ margin: '16px' }} onClick={handleEmailSend}>שלח</Button>
*/}
        </DialogActions>
      </Dialog>
      <Snackbar
